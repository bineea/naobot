...
naobot 是 KT2 LLM 桌面智能机器人软件 MVP。当前目标是让 KT2 具备宿主机 AgentScope Brain、FastAPI Dashboard、ESP32 MicroPython 固件骨架、机器人模拟器、协议模型、安全白名单、记忆/routine 和自动化测试。

已实现的大脑事实：`agentscope==2.0.4`；AgentScope Agent 使用 OpenAI-compatible 模型；L3 只允许输出 `goal`、`text`、`expression`、`skills`、`memory_suggestion`；L2 `BehaviorRuntime` 确定性编译并忽略 LLM `actions`；空 Toolkit 禁止 shell、文件、Python、MCP、硬件工具和自主长期记忆；未配置、超时、异常、非法输出均 fallback。
...
6. `docs/decisions/ADR-0003-agentscope-brain-runtime.md`：AgentScope Brain 运行时决策。
...
- 不允许 L3 大脑输出或信任 `actions`；兼容 actions 只能由 L2 确定性编译产生。
- 不允许为 AgentScope Brain 增加 shell、文件、Python、MCP 或硬件工具。
- 安全事件不得启用多 agent 组队；复杂请求最多三专家加产品负责人收敛。
- 不宣称真实硬件已验证，除非有明确实测记录。
