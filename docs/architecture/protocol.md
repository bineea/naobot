...
固件控制与媒体 client 复用同一增量帧解析器，TCP partial recv 会保留到下一轮；能重组 Host 下行的标准 text/binary continuation 序列，允许分片之间穿插 ping，并回复 pong，收到 close 时完成关闭握手。媒体重组后的单条消息上限为 256 KiB，控制消息上限为 32 KiB；非法 continuation 顺序、数据帧嵌套、RSV 位、服务端 masked 帧、超长消息或非法 control frame 会以 `1002`/`1009` 关闭。
...
Host heartbeat 默认每 2 秒独立发送，payload 包含 `source=host`、`host_ts_ms`、`agent_mode` 和 `last_intent_id`。固件用它刷新大脑在线时间并建立 intent deadline 的短期 Host 时钟锚点，不执行动作、不回 ack。
...
固件递归检查整个 intent payload 与每个 action/skill 参数，拒绝未知参数。`steps` 只能为 1-3，`wave.level` 只能为 1-2，`gentle_nudge.level` 只能为 1；face、tone 和 expression emotion 只能使用约定枚举。表情 `valence/pupil_offset_x` 为 -1 到 1，`arousal/eye_open/blink_rate` 为 0 到 1，`duration_ms` 为 0-5000。
