...
- 媒体队列有界；拥塞时优先淘汰旧视频，再淘汰非语音音频。媒体坏帧或后端异常不能阻断控制 heartbeat，也不能覆盖固件反射。
- 已实现音频半双工：TTS 期间只暂停麦克风上传，摄像头继续按常态 10 FPS/事件窗口 15 FPS 上传；固件在 `tts_end` 且播放缓冲排空后恢复麦克风，Host 在 TTS 完成后延迟 200 ms 恢复监听。
