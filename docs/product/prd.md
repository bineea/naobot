...
真实 XIAO ESP32-S3 Sense 硬件仍未验收。2026-07-24 已在 HEAD `240b4e9` 使用 MicroPython v1.28.0、ESP-IDF v5.5.1 和 esp32-camera v2.1.6 完成项目定制镜像的真实 C 编译：OTA 应用镜像 `micropython.bin` 为 1,840,304 字节，小于 `0x280000` app 分区，SHA-256 为 `cd8d890faeb24459d91ff1e30b1446434c7eba8d13d9475ed5b03f198a3b123b`；首次烧录合并镜像 `firmware.bin` 为 1,971,376 字节，SHA-256 为 `f7350d34442bebc80dea0854edb868727c48514dfcd088c6ecde1f0f1c7f8149`。该证据仅证明构建、链接和分区尺寸通过，尚未证明真实烧录、签名 OTA、失败回滚、OV2640、PDM/I2S、PSRAM、USB CDC、OLED、MPU6050、触摸、舵机或 30 分钟稳定性。
...
- 2026-07-24：修复 Stage 1 OTA pre-begin fail-closed 永久锁定——native abort 仅在 `IDLE` 且 active handle、目标分区、SHA context 均明确不存在时幂等确认成功，使 manifest/signature 阶段的瞬态顶层异常能完成 cleanup 并释放 OTA motion owner；WRITING、STAGED、ACTIVATED、真实 active/unknown 与 FAILED abort failure 重试契约保持不变，不修改 NVS 或 boot partition。HEAD `240b4e9` 已重新编译原生 C；`micropython.bin` 为 1,840,304 字节，SHA-256 为 `cd8d890faeb24459d91ff1e30b1446434c7eba8d13d9475ed5b03f198a3b123b`。真实设备 pre-begin 故障、签名升级、回滚和 30 分钟稳定性仍未验收。
